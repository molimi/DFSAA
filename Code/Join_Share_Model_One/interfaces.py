...
class BaseLassoNet(BaseEstimator, metaclass=ABCMeta):
    ...
    @abstractmethod
    def _convert_y(self, y_reg, y_cla) -> torch.TensorType:
        """Convert y to torch tensor"""
        raise NotImplementedError

    @abstractstaticmethod
    def _output_shape(cls, y_reg, y_cla):
    ...
    '''
    @abstractattr
    def criterion_reg(cls):
        raise NotImplementedError

    @abstractattr
    def criterion_cla(cls):
        raise NotImplementedError
    '''

    def _init_model(self, X, y_reg, y_cla):
        """Create a torch model"""
        output_shape_reg, output_shape_cla = self._output_shape(y_reg, y_cla)
        if self.class_weight is not None:
            assert output_shape_cla == len(self.class_weight)
        if self.torch_seed is not None:
            torch.manual_seed(self.torch_seed)
        self.model = LassoNet(
            X.shape[1], *self.hidden_dims, output_shape_reg, output_shape_cla, dropout=self.dropout
        ).to(self.device)
  

    def _cast_input(self, X, y_reg=None, y_cla=None):
        X = torch.FloatTensor(X).to(self.device)
        if y_reg is None and y_cla is None:
            return X
        y_reg, y_cla = self._convert_y(y_reg, y_cla)
        return X, y_reg, y_cla

    def fit(self, X, y_reg, y_cla, *, X_val=None, y_reg_val=None, y_cla_val=None):
        """Train the model.
        Note that if `lambda_` is not given, the trained model
        will most likely not use any feature.
        """
        self.path_ = self.path(X, y_reg, y_cla, X_val=X_val, y_reg_val=y_reg_val, y_cla_val=y_cla_val, return_state_dicts=False)
        return self
    ...
    def _train(
        ...
        *,
        ...
        def validation_obj():
            with torch.no_grad():
                y_model_reg_val, y_model_cla_val = model(X_val)
                return (
                    self.criterion(y_reg_val, y_cla_val, y_model_reg_val, y_model_cla_val).item()
        ...
        for epoch in range(epochs):
            ...
            for i in range(n_train // batch_size):
                ...
                def closure():
                    nonlocal loss
                    optimizer.zero_grad()
                    y_model_reg_train, y_model_cla_train = model(X_train[batch])
                    ans = (
                        self.criterion(y_reg_train[batch], y_cla_train[batch], y_model_reg_train, y_model_cla_train)
    ...
    def path(
        ...
        *,
        ...
        if sample_val:
            X_train, X_val, y_reg_train, y_reg_val, y_cla_train, y_cla_val = train_test_split(
                X, y_reg, y_cla, test_size=self.val_size, random_state=self.random_state)
        elif X_val is None:
            X_train, y_reg_train, y_cla_train = X_val, y_reg_val, y_cla_val = X, y_reg, y_cla
        else:
            X_train, y_reg_train, y_cla_train = X, y_reg, y_cla
        X_train, y_reg_train, y_cla_train = self._cast_input(X_train, y_reg_train, y_cla_train)
        X_val, y_reg_val, y_cla_val = self._cast_input(X_val, y_reg_val, y_cla_val)

        hist: List[HistoryItem] = []

        # always init model
        self._init_model(X_train, y_reg_train, y_cla_train)
            ...
            if is_dense and self.model.selected_count() < X.shape[1]:
                ...
                if current_lambda / lambda_start < 2:
                    warnings.warn(
                        ...
                        "might be too large.\n"
                        # f"Features start to disappear at {current_lambda:.3f}."   # 训练过程中会报错，可能lambda会出现问题
    ...
    def load(self, state_dict):
        ...
        if self.model is None:
            output_shape_reg, input_shape = state_dict["skip_reg.weight"].shape
            output_shape_cla, input_shape = state_dict["skip_cla.weight"].shape
            self.model = LassoNet(
                input_shape, *self.hidden_dims, output_shape_reg, output_shape_cla, dropout=self.dropout
            ).to(self.device)
...
class LassoNetRegressorClassifier(
    ...
    '''
    def criterion(self, y_reg, y_cla, y_reg_pred, y_cla_pred):
    ...
    def _convert_y(self, y_reg, y_cla):
    ...
    def predict(self, X):
        self.model.eval()
        with torch.no_grad():
            ans_reg, ans_cla = self.model(self._cast_input(X))
            ans_cla = ans_cla.argmax(dim=1)         # 输出类别
        if isinstance(X, np.ndarray):
...
def lassonet_path(X, y, task, *, X_val=None, y_val=None, **kwargs):
    ...
    if class_ is None:
        raise ValueError('task must be "classification," "regression," ')
    model = class_(**kwargs)
