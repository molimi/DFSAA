...
from prox import inplace_prox_D, inplace_prox_A, inplace_group_prox, prox
...
class LassoNet(nn.Module):
    def __init__(self, *dims, groups=None, dropout=None):
        ...
        # The return of genes and images
        """
        assert len(dims) > 2
    ...
    def selected_count(self):
        return self.input_mask().sum().item()   # Count the number of selected elements

    def cpu_state_dict(self):
        return {k: v.detach().clone().cpu() for k, v in self.state_dict().items()}


class RegressorClassifier(nn.Module):
    def __init__(self, *hidden_dims, dropout=None):
        """
        first dimension is input
        last dimension is output
        # Image and cognitive score regression, classification fusion model of diagnostic categories
        """
        ...
        self.layers_reg = nn.Linear(hidden_dims[-3], hidden_dims[-2])           # regression
        self.layers_cla = nn.Linear(hidden_dims[-3], hidden_dims[-1])           # classification
    ...
    def prox(self, *, lambda_, lambda_bar=0, M=1):                  # Combine classification and regression
        with torch.no_grad():
            inplace_prox_D(
                alpha=self.skip_reg,
                beta=self.skip_cla,
